...
public class BoardTestSuite {
    ...
    public void testAddTaskListAverageWorkingOnTask() {
        ...
        List<TaskList>  inProgressTasks = new ArrayList<>();
        inProgressTasks.add(new TaskList("In progress"));
        int sumOfWorkingDays = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(task -> Period.between(task.getCreated(),LocalDate.now()).getDays())
                .reduce(0, Integer::sum);

        int countOfTaskInProgress = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(t -> 1)
                .reduce(0, Integer::sum);
        double average = sumOfWorkingDays/countOfTaskInProgress;
        //Then
